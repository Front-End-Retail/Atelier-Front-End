...
const ReviewForm = ({ toggle, metaReviews, currentProductName }) => {
  ...
  const handleChange = (e) => {
    e.target.name === "username" && e.target.value.length > 0
      ...
      : e.target.name === "helpful"
      ? setRecommended(e.target.value === "true") //converts string to boolean
      : e.target.name === "body"
  ...
  const postReview = (reviewFormObj) => {
    axios.default
      .post(`${baseURL}/review`, reviewFormObj)
      .then((res) => {
      ...
      >
        <div classname="user-email-div">
          <label>
            ...
            <input
              required
              className="form-input review-modal-input"
          ...
          <label>
            ...
            <input
              ...
              type="email"
              name="email"
              placeholder="“Example: jackson125@email.com”"
            />
          </label>
        ...
        <div className="rating-recommend-container">
          ...
          <div className="form-star-rating">
            ...
            {formRating.length > 0 && <div>"{ratingDesc(formRating)}"</div>}
          </div>
        ...
        {metaReviews.characteristics &&
          Object.keys(metaReviews.characteristics).map((key, index) => {
            let objId = metaReviews.characteristics[key].id;
            return (
              <div key={index}>
                <div className="character-desc-container">
                  <p className="character-title">
                    {key}:{charDescription[key].length < 1 && " None selected"}{" "}
                  </p>
                  {charDescription[key].length > 0 && (
                    <p className="character-desc">{charDescription[key]}</p>
                  )}
                </div>
                <input
                  onChange={(e) => handleCharChange(key, e)}
                  type="radio"
                  value="1"
                  name={objId}
                />
        ...
        <div className="summary-input-div">
          <label>
            ...
            <input
              required
              className="form-input review-summary-form review-modal-input"
              onChange={handleChange}
              type="text"
              name="summary"
        ...
        {body.length < 50 ? (
          ...
          <div className="form-success">Minimum reached</div>
        )}
        {/* Upload files here */}
