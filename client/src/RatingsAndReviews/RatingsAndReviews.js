...
const RatingsAndReviews = ({
  ...
  const sortReviews = (name) => {
    axios.default
      .get(`${baseURL}/review`, {
        params: {
          specificURL: `reviews?product_id=${currentProductID}&count=500&sort=${name}`,
        },
      })
      .then((reviewData) => {
  ...
  const putRequest = (reviewId, path, clickedId) => {
    ...
    axios.default
      .put("/review/put", { review_id: reviewId, path: path })
      .then((data) => {
  ...
  const reviewRequest = () => {
    axios.default
      .get(`${baseURL}/review`, {
        params: {
          specificURL: `reviews?product_id=${currentProductID}&count=500&sort=relevance`,
        },
      })
      .then((reviewData) => {
        let reviewsArray = reviewData.data.results;
  ...
  return (
    <div id="center-reviews">
      <div id="title">
        <h2 id="review-title">RATINGS & REVIEWS</h2>
      </div>
