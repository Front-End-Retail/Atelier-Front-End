...
describe('overview', () => {
  beforeAll(() => {
    axios.get.mockImplementationOnce(() => Promise.resolve(dummyProduct));
    axios.get.mockImplementationOnce(() => Promise.resolve(dummyMetaData));
  });

  it('should render overview', async () => {
    render(<Overview currentProductID={37311} styles={dummyStyles} selectedStyle={dummySelectedStyle} />);
    expect(screen.getByTestId('overview')).toBeInTheDocument();
  })

  it('should render out 6 styles', async () => {
    render(<StyleSelector styles={dummyStyles} selectedStyle={dummySelectedStyle} />)
    waitFor(() => screen.findAllByTestId('styles'))
      .then(data => {
  ...
  it('should render product description', async () => {
    render(<ProductDescription currentProduct={dummyProduct} />)
  })
})